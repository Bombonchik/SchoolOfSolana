...
impl Calculator {
    ...
    pub fn addition(&mut self, x: i64, y: i64) -> Option<i64> {
        let add_op = Operation::new(x, y, OperationType::Addition);
        let result = add_op.operation_type.perform(x, y);
        self.history.push(add_op);
        result
    }
    ...
    pub fn subtraction(&mut self, x: i64, y: i64) -> Option<i64> {
        let sub_op = Operation::new(x, y, OperationType::Subtraction);
        let result= sub_op.operation_type.perform(x, y);
        self.history.push(sub_op);
        result
    }
    ...
    pub fn multiplication(&mut self, x: i64, y: i64) -> Option<i64> {
        let mul_op = Operation::new(x, y, OperationType::Multiplication);
        let result = mul_op.operation_type.perform(x, y);
        self.history.push(mul_op);
        result
    }
    ...
    pub fn show_history(&self) -> String {
        ...
        for op in &self.history {
            history_str += &format!(
                ...
                op.second_num,
                op.operation_type.perform(op.first_num, op.second_num).unwrap()
            );
            index += 1;
        }
        history_str
